...
def test_residual_conv_block():
    ...
    block = ResidualConvBlock(in_channels=3, out_channels=3, is_res=True)
...
def test_unet_down():
    ...
    down = UnetDown(in_channels=3, out_channels=6)
    x = torch.randn(1, 3, 64, 64)
    out = down(x)
...
def test_unet_up():
    ...
    up = UnetUp(in_channels=6, out_channels=3)
...
def test_context_unet():
    ...
    model = ContextUnet(in_channels=3, n_feat=64, n_classes=10)
...
def test_ddpm():
    print("Testing DDPM...")
    model = ContextUnet(in_channels=1, n_feat=64, n_classes=10)
    ddpm = DDPM(nn_model=model, betas=(1e-4, 0.02), n_T=100, device="cpu")
